 ...
 *****************************************************************************/
static void LCD_enableSegment(LCD_TypeDef * lcd, int com, int bitIndex)
{
	if(bitIndex >= 32) {
		bitIndex -= 32;
		com += 4;
 ...
 * @param bitvalue Bit value for segment
 *****************************************************************************/
static void LCD_disableSegment(LCD_TypeDef * lcd, int com, int bitIndex)
{
	if(bitIndex >= 32) {
		bitIndex -= 32;
		com += 4;
...
void LCD_Number(LCD_TypeDef *lcd, int value)
{
  ...
  for (digit = 0; digit < 4; digit++)
  {
    ...
    for (i = 0; i < 7; i++)
    {
      ...
      if (bitpattern & (1 << i))
      {
        LCD_enableSegment(lcd, com, bit);
      }
      else
      {
        LCD_disableSegment(lcd, com, bit);
      }
    }
    div = div * 10;
  }
...
void LCD_Symbol(LCD_TypeDef *lcd, lcdSymbol s, int on)
{
  ...
  switch (s)
  {
  case LCD_SYMBOL_OLIMEX:
    com = 3; bit = 15;
    break;
  case LCD_SYMBOL_ARR_UP:
    com = 1; bit = 3;
    break;
  case LCD_SYMBOL_ARR_DN:
    com = 3; bit = 3;
    break;
  case LCD_SYMBOL_ARR_L:
    com = 0; bit = 3;
    break;
  case LCD_SYMBOL_ARR_R:
    com = 2; bit = 3;
    break;
  case LCD_SYMBOL_MINUS:
    com = 0; bit = 39;
    break;
  case LCD_SYMBOL_PLUS:
    com = 3; bit = 39;
    break;
  case LCD_SYMBOL_u:
    com = 3; bit = 1;
...
void LCD_Battery(LCD_TypeDef *lcd, int batteryLevel)
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    bitvalue = EFMDisplay.Battery.bit[i];
