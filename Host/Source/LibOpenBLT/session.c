...
** \param     transportSettings Pointer to the structure with settings for the transport
**            layer that the protocol uses.
**
****************************************************************************************/
                 ...
                 void const * transportSettings)
{
   ...
   * because not every protocol might need additional settings. The same goes for the 
   * transportSettings parameter.
   */
  ...
  if (protocolPtr != NULL)
  {
    protocolPtr->Init(protocolSettings, transportSettings);
  }
} /*** end of SessionInit ***/
