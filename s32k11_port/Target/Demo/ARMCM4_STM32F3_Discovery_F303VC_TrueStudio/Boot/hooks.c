...
void UsbConnectHook(blt_bool connect)
{
  ...
  if (connect == BLT_TRUE)
  {
    /* To connect the USB device to the USB bus, a pull-up needs to be enabled on the
     * USB_DP line, such that it provides a logic high signal to the host. On the
     * STM32F3-Discovery board, there is already a pull-up that is always enabled. This
     * means that the USB_DP line is already high as long as it is not explicitely
     * configured as a digital output and set to logic low. Function HAL_PCD_MspInit()
     * already properly configures the USB_DP pin for USB operation, so nothing more
     * need to be done here.
     */
  }
  else
  {
    /* To disconnect the USB device from the USB bus, the USB_DP line needs to go to
     * logic low. On the STM32F3-Discovery board, there is a pullup on this line that is
     * always enabled. This means that there is only one way to make the USB_DP go logic
     * low, which is by reconfiguring the pin as a digital output and setting it low.
     */
