...
#include "stm32f2xx_ll_spi.h"                    /* STM32 LL SPI header                */
#include "boot.h"
...
void power_on (void)
{
  ...
  /* Configure SPI pins: SCK (PC10), MOSI (PC12) and MISO (PC11) */
  ...
  SPI_InitStruct.BaudRate = LL_SPI_BAUDRATEPRESCALER_DIV128; // 120MHz/4/128=234kHz < 400kHz
...
void set_max_speed(void)
{
  ...
  SPI_InitStruct.ClockPhase = LL_SPI_PHASE_1EDGE;
  SPI_InitStruct.NSS = LL_SPI_NSS_SOFT;
  SPI_InitStruct.BaudRate = LL_SPI_BAUDRATEPRESCALER_DIV2; // 120MHz/4/2=15MHz < 25MHz
  SPI_InitStruct.BitOrder = LL_SPI_MSB_FIRST;
  SPI_InitStruct.CRCCalculation = LL_SPI_CRCCALCULATION_DISABLE;
  SPI_InitStruct.CRCPoly = 7;
  LL_SPI_Init(SPI3, &SPI_InitStruct);
...
BYTE xchg_spi (BYTE dat)
{
  ...
  {
    result = LL_SPI_ReceiveData8(SPI3);
  }
