...
void NetApp(void)
{
  ...
  if (uip_newdata()) 
  {
    ...
    {
      /* the ResetParamsReqTcpInit flag is used as a boolean flag to indicate if the
       * bootloader should initialize the TCP/IP network stack. we just received a
       * firmware update request from the TCP/IP network and we are about to activate
       ...
       */
      /* trigger a system reset to activate the bootloader, while also setting the
       * ResetParamsReqTcpInit flag. note that IfxScuRcu_performReset() already
       * implements functionality to store data in the RSTCON2.USRINFO register and
       * therefore there is no need to explicitly use ResetParamsSetFlag()
       */
      IfxScuRcu_performReset(IfxScuRcu_ResetType_system, ResetParamsReqTcpInit);
    }
  }
} /*** end of NetApp ***/
