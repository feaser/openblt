/************************************************************************************//**
* \file         Source/ARMCM3_EFM32/mbrtu.c
...
void MbRtuInit(void)
{
  ...
  ASSERT_CT(BOOT_COM_MBRTU_CHANNEL_INDEX == 1);
  /* max baudrate for LEUART is 9600 bps */
  ASSERT_CT(BOOT_COM_MBRTU_BAUDRATE <= 9600);
...
static blt_bool MbRtuReceiveByte(blt_int8u *data)
{
  ...
  if ((LEUART1->IF & LEUART_IF_RXDATAV) != 0)
  {
    ...
    data_and_status = LEUART_RxExt(LEUART1);
     ...
     * a frame error check.
     */
    if ((data_and_status & USART_RXDATAX_FERR) != 0U)
    {
    ...
    if ((data_and_status & USART_RXDATAX_PERR) != 0U)
    {
...
static void MbRtuTransmitByte(blt_int8u data, blt_bool end_of_packet)
{
  ...
  else
  {
     ...
     */
    while ((LEUART1->STATUS & LEUART_STATUS_TXC) == 0)
    {
      /* keep the watchdog happy. */
      CopService();
      /* break loop upon timeout. this would indicate a hardware failure. */
...
blt_int16u MbRtuFreeRunningCounterGet(void)
{
  ...
  if (initialized == BLT_FALSE)
  {
     ...
     */
    counts_per_ten_us = SystemCoreClockGet() / 100000UL;
