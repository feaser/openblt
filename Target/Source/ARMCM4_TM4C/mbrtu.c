/************************************************************************************//**
* \file         Source/ARMCM4_TM4C/mbrtu.c
...
void MbRtuInit(void)
{
  ...
  /* configure the number of databits. this also default to no parity and 1 stopbit */
  uartConfig = UART_CONFIG_WLEN_8;
  /* configure the parity */
  #if (BOOT_COM_MBRTU_PARITY == 1)  
  uartConfig |= UART_CONFIG_PAR_ODD;
  #elif (BOOT_COM_MBRTU_PARITY == 2)  
  uartConfig |= UART_CONFIG_PAR_EVEN;
  #endif  
  /* configure the stop bits */
  #if (BOOT_COM_MBRTU_STOPBITS == 2)
  uartConfig |= UART_CONFIG_STOP_TWO;
  #endif  
  /* configure the UART0 baudrate and communication parameters */
  UARTConfigSetExpClk(UART0_BASE, SysCtlClockGet(), BOOT_COM_MBRTU_BAUDRATE, uartConfig);
...
static blt_bool MbRtuReceiveByte(blt_int8u *data)
{
  ...
  /* check if a new byte was received by means of the RXFE-bit. */
  if (UARTCharsAvail(UART0_BASE) == true)
  {
    /* update the result */
    result = BLT_TRUE;
    /* read out the newly received data, including its reception status flags. */
    data_and_status = HWREG(UART0_BASE + UART_O_DR);
     ...
     * a frame error check.
     */
    if ((data_and_status & UART_DR_FE) != 0U)
    {
    ...
    if ((data_and_status & UART_DR_PE) != 0U)
    {
...
static void MbRtuTransmitByte(blt_int8u data, blt_bool end_of_packet)
{
  ...
  {
    /* wait for busy flag to be reset. this indicated that the last byte has completely 
     * left the shift register, including its stop bit. this is needed for the last byte,
     * otherwise the transceiver's transmit output gets disabled with 
...
blt_int16u MbRtuFreeRunningCounterGet(void)
{
  ...
  if (initialized == BLT_FALSE)
  {
     ...
     */
    counts_per_ten_us = SysCtlClockGet() / 100000UL;
